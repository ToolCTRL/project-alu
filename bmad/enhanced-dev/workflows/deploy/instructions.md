...
Execute deployment pipeline: tests → build → optional git/server steps based on project setup.
...
2. Ask user about deployment:
   Display: "📦 Deployment Setup"
   Display: "Git detected: {has_git}"

   Ask: "Möchtest du nach erfolgreichen Tests deployen?"
   Options:
     a) Nein, nur Tests ausführen
...
3. If deploy_mode = "b" or "c":
   Ask: "Welches Command soll ausgeführt werden?"
   Examples:
     - npm run build
     - npm run deploy
     - docker-compose up -d
...
```yaml
Check prerequisites:
  ✓ Status file exists at {status_file}
  ✓ If {has_git} = true: Check git status
...
Display summary:
  ...
  → Git: {git_action if has_git, else "not available"}
...
Display results:
  ...
  {if failing_count > 0}:
    ✗ {failing_count} tests failing
    Display failed test details

If {require_tests_pass} = true AND any tests fail:
  → Display: "❌ Deployment abgebrochen - Tests müssen erfolgreich sein"
  → STOP workflow
  → Exit with error

If {test_coverage_threshold} configured AND coverage < threshold:
  → Display: "⚠️  Warning: Coverage {coverage}% unter Threshold {threshold}%"
  → Ask: "Trotzdem fortfahren?"
    If NO: STOP workflow

Display: "✅ Alle Tests erfolgreich"
```
...
If {pre_deploy_command} configured:
  ...
  If fails:
    → Display: "❌ Build fehlgeschlagen"
    → Display build errors
    → STOP workflow

  Display: "✅ Build erfolgreich"
```
...
If {deploy_mode} = "b" or "c":
  ...
  If fails:
    → Display: "❌ Deployment fehlgeschlagen"
    → Display error output
    → Ask: "Trotzdem Git Commit erstellen?" (if {has_git})
    → Update status: deploy_failed
    → STOP workflow (or continue to git if user confirms)

  Track deploy end time: {deploy_end}
  Calculate duration: {deploy_duration}

  Display: "✅ Deployment erfolgreich ({deploy_duration})"
```
...
If {has_git} = true AND {git_action} != "a" (not "Nein"):
  ...
  4. If {git_action} = "b" (committen und pushen):
     Display: "⬆️  Pushing to remote..."

     Get current branch: git branch --show-current → {current_branch}
     Get remote: git remote → {remote} (usually "origin")

     Execute: git push {remote} {current_branch}

     If fails:
       → Display: "❌ Git push fehlgeschlagen"
       → Display common fixes:
         - git pull --rebase
         - Check network connection
         - Check permissions
       → Commit exists locally but not pushed
       → Exit with warning

     Display: "✅ Pushed to {remote}/{current_branch}"
```
...
Tests:
  ...
  {if failing_count > 0}: ✗ {failing_count} failing
  {if coverage}: 📈 Coverage: {coverage}%
  ⏱️  Duration: {test_duration}
...
{if deploy_mode != "a"}:
  Deployment:
    ...
    ⏱️  Duration: {deploy_duration}

{if has_git and git_action != "a"}:
  Git:
    📝 Commit: {commit_hash}
    {if git_action = "b"}: ⬆️  Pushed to {remote}/{branch}
