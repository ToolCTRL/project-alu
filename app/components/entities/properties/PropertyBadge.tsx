...
function isNumberType(type: PropertyType): boolean {
  return [PropertyType.NUMBER, PropertyType.RANGE_NUMBER].includes(type);
}

function isDateType(type: PropertyType): boolean {
  return [PropertyType.DATE, PropertyType.RANGE_DATE].includes(type);
}

function isSelectType(type: PropertyType): boolean {
  return [PropertyType.SELECT, PropertyType.MULTI_SELECT, PropertyType.MULTI_TEXT].includes(type);
}

function getPropertyIcon(type: PropertyType, className?: string) {
  ...
  if (type === PropertyType.BOOLEAN) {
    return <CheckIcon className={clsx(className, "h-5 w-5 shrink-0")} />;
  }
  ...
  if (type === PropertyType.MEDIA) {
    return <PaperClipIcon className={clsx(className, "h-5 w-5 shrink-0")} />;
  }

  if (type === PropertyType.FORMULA) {
    return <ExperimentIconFilled className={clsx(className, "h-5 w-5 shrink-0")} />;
  }
...
export default function PropertyBadge({ title, type, className }: Props) {
  ...
  return <>{getPropertyIcon(type, className)}</>;
}
