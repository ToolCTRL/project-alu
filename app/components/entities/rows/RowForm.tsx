...
import { useNavigation, useParams, useSearchParams, useSubmit } from "react-router";
    ...
    <div className={className}>
      ...
      {getRows(relationship).length === 0 ? (
        <button
          ...
          disabled={readOnly}
          className={clsx(
            "border-border relative block w-full rounded-lg border-2 border-dashed p-4 text-center",
            readOnly ? "bg-secondary/90 cursor-not-allowed" : "bg-background hover:border-border focus:ring-2 focus:ring-gray-500 focus:outline-hidden"
          )}
        >
