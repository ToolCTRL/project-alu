import { useLocation, useSubmit, useNavigation } from "react-router";
        ...
        <>
          <div className="relative px-1">
            ...
            <span className="bg-secondary absolute -bottom-0.5 -right-1 rounded-tl px-0.5 py-px">
              <ChatAltIcon className="text-muted-foreground h-4 w-4" aria-hidden="true" />
            </span>
          </div>
          <div className="group min-w-0 flex-1 space-y-0.5">
            ...
            <div className="text-muted-foreground text-sm font-normal">
              {item.comment.isDeleted ? (
                <p className="text-muted-foreground italic">{t("shared.commentDeleted")}</p>
              ) : (
                <div className="prose border-border rounded-md border border-dashed p-2 text-sm">
                  <div dangerouslySetInnerHTML={{ __html: marked(item.comment.value) }} />
                </div>
              )}
            </div>
            <div className="text-muted-foreground flex items-center justify-between space-x-1 font-medium">
              ...
              <div className="group flex items-center space-x-2">
                {!item.comment.isDeleted && item.comment.createdByUserId === user?.id && (
                  <button
                    ...
                    disabled={isDeleting}
                    onClick={() => onDeleted()}
                    className="text-muted-foreground hover:text-muted-foreground invisible p-0.5 focus:outline-hidden group-hover:visible"
                  >
                ...
                <button
                  ...
                  className={clsx(
                    "text-muted-foreground hover:text-muted-foreground p-0.5 focus:outline-hidden",
