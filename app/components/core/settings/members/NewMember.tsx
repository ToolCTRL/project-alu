...
import { Form, useActionData, useLocation, useNavigate, useNavigation, useParams } from "react-router";
    ...
    <CheckPlanFeatureLimit item={featurePlanUsage}>
      <Form method="post" className="space-y-4">
        <div className="grid grid-cols-2 gap-2">
          ...
          <div className="col-span-2">
            ...
            <div className="mt-1 flex w-full rounded-md shadow-2xs">
              <input
                ...
                disabled={loading}
                className={clsx(
                  "focus:border-theme-500 focus:ring-ring border-border block w-full min-w-0 flex-1 rounded-md lowercase sm:text-sm",
                  loading && "bg-secondary/90 cursor-not-allowed"
                )}
              />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="mt-1 flex w-full rounded-md shadow-2xs">
              <input
                ...
                className={clsx(
                  "focus:border-theme-500 focus:ring-ring border-border block w-full min-w-0 flex-1 rounded-md sm:text-sm",
                  loading && "bg-secondary/90 cursor-not-allowed"
                )}
              />
            </div>
          ...
          {/*User Last Name */}
          <div>
            <label htmlFor="last-name" className="text-foreground/80 block truncate text-xs font-medium">
        ...
        <div className="mt-4 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            <button
              disabled={loading}
              className={clsx(
                "focus:ring-ring hover:bg-secondary border-border bg-background text-foreground/80 inline-flex items-center space-x-2 border px-3 py-2 font-medium shadow-2xs focus:outline-hidden focus:ring-2 focus:ring-offset-2 sm:rounded-md sm:text-sm",
                loading && "bg-secondary/90 cursor-not-allowed"
              )}
              type="button"
              onClick={close}
            ...
            <button
              disabled={loading}
              className={clsx(
                "bg-primary hover:bg-primary/90 focus:ring-primary inline-flex items-center space-x-2 border border-transparent px-3 py-2 font-medium text-white shadow-2xs focus:outline-hidden focus:ring-2 focus:ring-offset-2 sm:rounded-md sm:text-sm",
