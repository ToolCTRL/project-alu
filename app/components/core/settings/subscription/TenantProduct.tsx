...
export default function TenantProduct({ item, onCancel }: Readonly<Props>) {
  ...
  return (
    <div className="border-border bg-background group space-y-3 rounded-lg border p-4 shadow-2xs">
      <div className="flex w-full justify-between">
        <div className="text-foreground text-base font-medium">{getPlanTitle()}</div>
          ...
          <div>
            {!item.cancelledAt && (
              <button
                ...
                className="hover:bg-secondary hover:border-border text-foreground/80 invisible border border-transparent px-2  py-1 text-sm hover:rounded-md group-hover:visible group-hover:underline"
              >
            ...
            return (
              <Fragment key={price.id}>
                {price.subscriptionPrice && (
                  <div className="text-muted-foreground flex items-baseline space-x-1 text-sm">
      ...
      {rootData.debug && (
        <div className="border-border bg-secondary block space-y-2 rounded-md border border-dashed p-3 text-xs">
