  ...
  return (
    <div>
                ...
                .map((plan) => {
                  return (
                    <div key={plan.id}>
                      ...
                      >
                        {isCurrent(plan) && (
                          <div className="absolute top-0 -translate-y-1/2 transform rounded-full bg-teal-500 px-4 py-1.5 text-xs font-semibold uppercase tracking-wide text-white">
                            {t("shared.current")}
                          </div>
                        )}
                        <div className="flex-1 space-y-2">
                          ...
                          <div className="shrink-0 truncate">
                            ...
                            {plan.model === PricingModel.PER_SEAT && <span className="text-muted-foreground">/{t("pricing.seat").toLowerCase()}</span>}
                            {(() => {
                              if (getPrice(plan)?.billingPeriod === SubscriptionBillingPeriod.MONTHLY) {
                                return <span className="text-muted-foreground lowercase">/{t("pricing.MONTHLYShort")}</span>;
                              } else if (getPrice(plan)?.billingPeriod === SubscriptionBillingPeriod.YEARLY) {
                                return <span className="text-muted-foreground lowercase">/{t("pricing.YEARLYShort")}</span>;
                              } else if (getPrice(plan)?.billingPeriod === SubscriptionBillingPeriod.ONCE) {
                                return <span className="text-muted-foreground lowercase">/{t("pricing.once")}</span>;
                              }
                            })()}
                          </div>
                                  ...
                                  <li key={feature.name}>
                                    <div className="flex items-center">
                                      {feature.type === SubscriptionFeatureLimitType.NOT_INCLUDED ? (
                                        <svg className="text-gray-300 h-4 w-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                      ...
                                      <span className="ml-3 truncate text-sm">
                                        <span>{t(feature.title, { 0: feature.value })}</span>
                                      </span>
                                    </div>
                                  </li>
                                );
                          ...
                          <div className="shrink-0 space-y-2 pt-4">
                            <button
                              ...
                              className={clsx(
                                "inline-flex w-full items-center justify-center rounded-lg border px-4 py-2 text-sm transition-colors focus:outline-hidden focus:ring-2 focus:ring-offset-2",
      ...
      <Modal className="sm:max-w-xs" open={showQuantityModal} setOpen={setShowQuantityModal}>
        <InputNumber name="quantity" title={t("pricing.seats")} value={quantity} setValue={(e) => setQuantity(Number(e))} required min={1} />
        {selectedPlan && (
