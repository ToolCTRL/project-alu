...
export default function NewTenantSelect({ onOpenCommandPalette }: Readonly<Props>) {
  ...
  return (
    <button type="button" onClick={onOpenCommandPalette} className="text-foreground group flex w-full shrink-0 focus:outline-hidden">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center truncate text-left">
            {appData?.currentTenant?.icon ? (
              <img className="inline-block h-9 w-9 shrink-0 rounded-full shadow-2xs" src={appData?.currentTenant?.icon} alt={appData?.currentTenant?.name} />
            ) : (
