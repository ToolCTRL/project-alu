...
export default function ActionResultModal({ actionResult, className, actionData, showSuccess = true, showError = true, onClosed }: Readonly<Props>) {
  ...
  useEffect(() => {
    if (status?.success) {
      setData({
        title: status.success.title ?? t("shared.success"),
        description: status.success.description,
        closeText: status.success.closeText ?? t("shared.close"),
      });
      setOpen(true);
    } else if (status?.error) {
      setData({
        title: status.error.title ?? t("shared.error"),
        description: status.error.description,
        closeText: status.error.closeText ?? t("shared.close"),
      });
      setOpen(true);
    }
  }, [status, t]);
  ...
  return (
    <Transition show={open} as={Fragment}>
      <Dialog
        as="div"
        className={clsx(className, "fixed inset-0 z-50 overflow-y-auto transition duration-300 ease-out data-[closed]:opacity-0")}
        transition
            ...
            <DialogPanel className="bg-background inline-block w-full transform overflow-hidden rounded-lg px-4 pt-5 pb-4 text-left align-bottom shadow-xl transition-all sm:my-8 sm:max-w-sm sm:p-6 sm:align-middle">
              ...
              <div className="mt-5 sm:mt-6">
                <button
                  type="button"
                  className={clsx(
                    "inline-flex w-full justify-center rounded-md border border-transparent px-4 py-2 text-base font-medium text-white shadow-2xs focus:ring-2 focus:ring-offset-2 focus:outline-hidden sm:text-sm",
