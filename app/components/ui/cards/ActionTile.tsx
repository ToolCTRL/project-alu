...
function TileContent({ title, description, hint, icon }: Pick<ActionTileProps, "title" | "description" | "hint" | "icon">) {
  return (
...
export function ActionTile(props: ActionTileProps) {
  const { title, description, hint, icon, tone = "accent", onClick, href, actionLabel = "Öffnen", className, disableTilt } = props;
  ...
  const content = (
    <>
      <TileContent title={title} description={description} hint={hint} icon={icon} />
  ...
  if (href) {
    return (
      <a href={href} className={commonClasses} {...tiltProps}>
        {content}
      </a>
    );
  }
  ...
  return (
    <button type="button" className={commonClasses} onClick={onClick} {...tiltProps}>
      {content}
    </button>
  );
}
