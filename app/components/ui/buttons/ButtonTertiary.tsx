      ...
      {(() => {
        if (!to || disabled) {
          return (
            <Button
              variant={"ghost"}
              className={clsx(className, destructive && "text-red-500 hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-900 dark:hover:text-white")}
              onClick={
                confirmation
                  ? () => {
              ...
              // className={clsx(
              //   className,
              //   "focus:ring-accent-300 mx-1 my-2 inline-flex items-center space-x-2 border-b border-transparent text-sm font-medium focus:rounded-md focus:outline-hidden focus:ring-2 focus:ring-offset-2",
