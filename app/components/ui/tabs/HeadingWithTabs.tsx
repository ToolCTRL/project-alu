...
export default function HeadingWithTabs({ tabs }: { readonly tabs: { name: string; href: string }[] }) {
  ...
  return (
    <div className="w-full sm:flex sm:items-baseline">
      <h3 className="text-foreground text-lg font-medium leading-6">Issues</h3>
        ...
        <nav className="-mb-px flex space-x-8">
          {tabs.map((tab, idx) => (
            <a
              ...
              href={tab.href}
              className={clsx(
                isCurrent(idx) ? "border-theme-500 text-theme-600" : "hover:border-border text-muted-foreground hover:text-foreground/80 border-transparent",
