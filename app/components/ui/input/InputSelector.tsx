  ...
  return (
    <Fragment>
      {/* <Listbox value={selected} defaultValue={defaultSelected} onChange={setSelected} disabled={disabled || readOnly}>
        {({ open }) => (
          <div className={clsx(className, darkMode ? "text-foreground dark:text-gray-50" : "text-foreground")}>
            {withLabel && title && (
            ...
            <div className="relative">
              <Listbox.Button
                ...
                ref={button}
                className={clsx(
                  "relative w-full cursor-default rounded-md border border-border py-2 pl-3 pr-10 text-left shadow-2xs focus:border-border focus:outline-hidden focus:ring-1 focus:ring-gray-500 sm:text-sm",
                  darkMode ? "text-foreground dark:text-gray-50" : "text-foreground",
                  disabled || readOnly ? "cursor-not-allowed bg-secondary/90" : "bg-background hover:bg-secondary focus:bg-secondary",
                  borderless && "border-transparent",
                  darkMode && "dark:border-gray-800 dark:bg-foreground/90"
                )}
              >
              ...
              <Transition show={open} as={Fragment} leave="transition ease-in duration-100" leaveFrom="opacity-100" leaveTo="opacity-0">
                <Listbox.Options
                  ...
                  className={clsx(
                    "absolute z-10 mt-1 max-h-72 w-full overflow-auto rounded-md bg-background py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-hidden sm:text-sm",
                    darkMode && "dark:bg-foreground/90"
                  )}
                >
                    ...
                    <div className="flex rounded-md p-2">
                      <div className="relative flex grow items-stretch focus-within:z-10">
                        ...
                        <input
                          ...
                          className="block w-full rounded-none rounded-l-md border border-border bg-background px-3 py-2 pl-10 text-sm focus:border-border focus:outline-hidden focus:ring-gray-300 sm:text-sm"
                        />
                      </div>
                      {onNew && (
                        <button
                          ...
                          type="button"
                          onClick={onNew}
                          className="relative -ml-px inline-flex items-center space-x-2 rounded-r-md border border-border bg-secondary px-2 py-2 text-sm font-medium text-foreground/80 hover:bg-secondary/90 focus:border-border focus:outline-hidden focus:ring-1 focus:ring-gray-500"
                        >
                    ...
                    <Listbox.Option
                      ...
                      className={({ active }) =>
                        clsx(
                          ...
                          !item.disabled && !active && "text-foreground",
                          item.disabled && "cursor-not-allowed bg-secondary/90 text-muted-foreground",
                          darkMode && !item.disabled && active && "dark:bg-gray-500 dark:text-black",
                          darkMode && !item.disabled && !active && "dark:text-gray-50",
                          darkMode && item.disabled && "cursor-not-allowed dark:bg-gray-900 dark:text-muted-foreground"
                        )
                      }
                      value={item}
                    >
                      {({ selected, active }) => (
                        <>
                          ...
                          {selected ? (
                            <span className={clsx(active ? "text-white" : "text-muted-foreground", "absolute inset-y-0 right-0 flex items-center pr-4")}>
                  ...
                  {withSearch && filteredItems().length === 0 && <div className="px-3 py-2 text-sm text-muted-foreground">{t("shared.noRecords")}</div>}
                </Listbox.Options>
              </Transition>
            </div>
          </div>
