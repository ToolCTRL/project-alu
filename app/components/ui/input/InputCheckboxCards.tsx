...
export default function InputCheckboxCards({
  ...
  useEffect(() => {
    if (value && !isEqual([...selected].toSorted((a, b) => String(a).localeCompare(String(b))), [...value].toSorted((a, b) => String(a).localeCompare(String(b))))) {
      setSelected(value);
    }
  ...
  useEffect(() => {
    if (onChange && !isEqual([...selected].toSorted((a, b) => String(a).localeCompare(String(b))), value ? [...value].toSorted((a, b) => String(a).localeCompare(String(b))) : [])) {
      onChange(selected);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selected]);
          ...
          return (
            <li key={item.value}>
              <label
                ...
                className={clsx(
                  clsx(
                    isChecked(item.value) ? "ring-ring ring-2" : "",
                    "relative flex rounded-lg p-3 shadow-2xs select-none focus:outline-hidden",
              ...
              >
                ...
                <div className="block w-full">
                  ...
                  <div className="flex w-full justify-between truncate">
                    ...
                    <CheckFilledCircleIcon className={clsx("shrink-0", !isChecked(item.value) && "invisible", "text-primary h-5 w-5")} aria-hidden="true" />
                    <span
                      className={clsx(isChecked(item.value) ? "ring-ring border" : "border", "pointer-events-none absolute -inset-px rounded-lg")}
                      aria-hidden="true"
                    />
                  </div>
                  {["nameAndValue", "value"].includes(displayType) && <div className="w-full truncate text-sm">{item.value}</div>}
                </div>
              </label>
            </li>
          );
