    ...
    <Listbox value={selected} onChange={setSelected} disabled={disabled || readOnly}>
      {({ open }) => (
        <div className={clsx(className, "text-slate-50")}>
          {withLabel && title && (
            <Listbox.Label htmlFor={name} className="text-muted-foreground mb-1 flex justify-between space-x-2 text-xs font-medium">
          ...
          <div className="relative">
            <Listbox.Button
              ...
              ref={button}
              className={clsx(
                "focus:border-border focus:ring-accent-700 relative w-full cursor-default rounded-sm border border-slate-800 bg-slate-800 py-2 pr-10 pl-3 text-left text-slate-50 shadow-2xs hover:bg-slate-700 focus:ring-1 focus:outline-hidden sm:text-sm",
            ...
            <Transition show={open} as={Fragment} leave="transition ease-in duration-100" leaveFrom="opacity-100" leaveTo="opacity-0">
              <Listbox.Options
                ...
                className={clsx(
                  "ring-opacity-5 absolute z-10 mt-1 max-h-72 w-full overflow-auto rounded-sm bg-slate-800 py-1 text-base text-white shadow-lg ring-1 ring-black focus:outline-hidden sm:text-sm"
                )}
              >
                  ...
                  <div className="flex rounded-sm p-2">
                    <div className="relative flex grow items-stretch focus-within:z-10">
                      ...
                      <input
                        ...
                        className="focus:border-accent-300 bg-background border-border block w-full rounded-none rounded-l-md border px-3 py-2 pl-10 text-sm focus:ring-slate-300 focus:outline-hidden sm:text-sm"
                      />
                    </div>
                    {onNew && (
                      <button
                        ...
                        type="button"
                        onClick={onNew}
                        className="focus:border-border focus:ring-ring bg-secondary border-border relative -ml-px inline-flex items-center space-x-2 rounded-r-md border px-2 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100 focus:ring-1 focus:outline-hidden"
                      >
                  ...
                  <Fragment key={group.name || 'no-name'}>
                    {group.name && (
                      <Listbox.Option
                        disabled
                        className={({ active }) =>
                          clsx("relative cursor-default py-1 pr-9 pl-3 select-none", active ? "bg-accent-500 text-black" : "text-slate-50")
                        }
                        value={group}
                      >
                        {({ selected, active }) => (
