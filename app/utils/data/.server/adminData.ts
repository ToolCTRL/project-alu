...
export async function loadAdminData({ request, t }: { request: Request; t: TFunction }, time: TimeFunction) {
  ...
  const tenantId = tenantIdParam ? await time(getTenantByIdOrSlug(tenantIdParam), "getTenantByIdOrSlug") : null;
  ...
  const currentTenant = tenantId ? myTenants.find((f) => f.id === tenantId || f.slug === tenantId || f.name === tenantId) ?? null : null;
  if (tenantId && !currentTenant) {
    throw Response.json({ error: "Unauthorized tenant" }, { status: 401 });
  }

  const { allPermissions, superAdminRole, entities, entityGroups, allRoles, myGroups, onboardingSession } = await time(
    promiseHash({
      allPermissions: getPermissionsByUser(userInfo.userId, tenantId ?? null),
      superAdminRole: getUserRoleInAdmin(userInfo.userId, DefaultAdminRoles.SuperAdmin),
      entities: getAllEntities({ tenantId: tenantId ?? null }),
      entityGroups: getAllEntityGroups(),
      allRoles: getAllRolesWithoutPermissions(tenantId ? "app" : "admin"),
      myGroups: getMyGroups(user.id, tenantId ?? null),
      onboardingSession: OnboardingService.getUserActiveOnboarding({ userId: user.id, tenantId: tenantId ?? null, request }),
    }),
