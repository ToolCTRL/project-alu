...
function getBlockPreviews({ t, entities }: { t: TFunction; entities: EntityWithDetails[] }) {
  ...
  entities.forEach((entity) => {
    cruds.views.push({
      ...
      // icon: <PlusIcon className="mx-auto h-4 w-4 text-foreground" />,
    });
  });
  const previews: NoCodeEntityViewsDto[] = [
    ...
    {
      title: "Stats",
      description: "Stats, Charts, Reports...",
      views: [
        {
          name: "Count",
...
function getEntityPreviews(entity: EntityWithDetails, rows: RowWithDetails[]) {
      ...
      views: [
        {
          ...
          icon: <ListIcon className="text-foreground mx-auto h-4 w-4" />,
        },
        {
          name: "List (default)",
        ...
        },
        {
          ...
          icon: <PlusIcon className="text-foreground mx-auto h-4 w-4" />,
        },
        {
          name: "Overview",
          ...
          icon: <PrototypeIcon className="text-foreground mx-auto h-4 w-4" />,
        },
        {
          name: "Edit",
          ...
          error: rows.length === 0 ? "No rows" : undefined,
          icon: <EditIcon className="text-foreground mx-auto h-4 w-4" />,
        },
        {
          name: "Import",
          description: "Imports entity rows from a .csv",
          href: `${entity.slug}/import`,
          icon: <UploadIcon className="text-foreground mx-auto h-4 w-4" />,
        },
        {
          name: "Export",
          ...
          reloadDocument: true,
          icon: <DownloadIcon className="text-foreground mx-auto h-4 w-4" />,
        },
        {
          name: "Tags",
          ...
          error: rows.length === 0 ? "No rows" : undefined,
          reloadDocument: true,
          icon: <TagsIcon className="text-foreground mx-auto h-4 w-4" />,
        },
        {
          name: "Share",
          ...
          error: rows.length === 0 ? "No rows" : undefined,
          reloadDocument: true,
          icon: <ShareIcon className="text-foreground mx-auto h-4 w-4" />,
        },
        {
          name: "Public URL",
          description: "Share a row to the world",
          href: publicRow ? `/public/${entity.slug}/${rows[0].id}` : undefined,
          error: publicRow ? undefined : "No public rows",
          reloadDocument: true,
          icon: <LinkIcon className="text-foreground mx-auto h-4 w-4" />,
        },
        // {
        //   name: "Relationships",
        //   description: "Parents and children relationships view",
        //   href: `${entity.slug}/relationships`,
        //   icon: <TreeStructureIcon className="text-foreground mx-auto h-4 w-4" />,
        //   underConstruction: true,
        //   enterprise: true,
        // },
      ],
  ...
  if (entity.isAutogenerated && entity.type !== "system") {
    // Entity is autogenerated and not system type - no error
  } else {
