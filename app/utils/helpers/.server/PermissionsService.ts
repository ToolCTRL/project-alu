...
import { findPermissionByUser, countUserPermission, findUserRolesByIds, getUserRoles, getUserRoleInAdmin } from "~/utils/db/permissions/userRoles.db.server";
...
export async function verifyUserHasPermission(request: Request, permissionName: DefaultPermission, tenantId: string | null = null) {
  ...
  const effectiveTenantId = tenantId !== null ? tenantId : tenantIdFromUrl ?? null;

  // Map operator permissions to tenant-scoped permissions when a tenant context is present
  let effectivePermission: DefaultPermission = permissionName;
  if (effectiveTenantId) {
    if (permissionName.startsWith("admin.entities.")) {
      effectivePermission = permissionName.replace("admin.entities.", "tenant.entities.") as DefaultPermission;
    } else if (permissionName.startsWith("admin.workflows.")) {
      effectivePermission = permissionName.replace("admin.workflows.", "tenant.workflows.") as DefaultPermission;
    } else if (permissionName.startsWith("admin.pages.")) {
      effectivePermission = permissionName.replace("admin.pages.", "tenant.pages.") as DefaultPermission;
    }
  }

  const isSuperAdmin = await getUserRoleInAdmin(userInfo.userId, DefaultAdminRoles.SuperAdmin);
  const permission = await getPermissionName(effectivePermission);
  if (permission) {
    if (isSuperAdmin && effectivePermission.startsWith("tenant.")) {
      return true;
    }
    const userPermission = (await countUserPermission(userInfo.userId, effectiveTenantId, effectivePermission)) > 0;
    if (!userPermission) {
      if (effectiveTenantId) {
        throw redirect(`/unauthorized/${effectivePermission}/${effectiveTenantId}/?redirect=${url.pathname}`);
      } else {
        throw redirect(`/unauthorized/${effectivePermission}?redirect=${url.pathname}`);
      }
    }
  }
  return true;
...
export async function getUserRowPermission(row: RowWithDetails, tenantId?: string | null, userId?: string): Promise<RowPermissionsDto> {
  ...
  if (tenantId === null && userId) {
    const isSuperAdmin = await getUserRoleInAdmin(userId, DefaultAdminRoles.SuperAdmin);
    if (isSuperAdmin) {
      isOwner = true;
    }
