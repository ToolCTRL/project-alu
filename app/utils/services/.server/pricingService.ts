...
export async function addTenantProductsFromCheckoutSession({
  ...
  if (!checkoutSession) {
    ...
    if (!appConfiguration.subscription.multiple) {
      const hasSubscribed = checkoutSession.products.some((product) => product.subscription);
      const activeSubscriptions = await getActiveTenantSubscriptions(tenant.id);
      const recurringSubscriptions = activeSubscriptions?.products.filter((f) => f.stripeSubscriptionId) || [];
      console.log("Cancelling existing subscriptions", {
        hasSubscribed,
        recurringSubscriptions: recurringSubscriptions.map((f) => t(f.subscriptionProduct.title)),
      });
      if (hasSubscribed && recurringSubscriptions.length > 0) {
        await Promise.all(
          recurringSubscriptions.map(async (recurringSubscription) => {
            console.log("Cancelling subscription", { recurringSubscription: t(recurringSubscription.subscriptionProduct.title) });
            return await cancelTenantSubscription(recurringSubscription.id, { tenantId, userId: user.id, t });
          })
        );
      }
    }
