...
async function processEmailAttachments(emailWithAttachments: any) {
  ...
  await Promise.all(
    emailWithAttachments.attachments.map(async (attachment: any) => {
...
async function handleInboundEmail(email: any) {
  ...
  await processEmailAttachments(emailWithAttachments);
...
export const action: ActionFunction = async ({ request }) => {
  try {
    if (request.method === "POST") {
      ...
      return handleInboundEmail(email);
    }
