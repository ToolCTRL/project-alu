...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  if (!sig) {
    return Response.json("No signature", { status: 400 });
  }
  let event;
...
async function updateTenantSubscription({ request, t, stripeSubscriptionId }: { request: Request; t: TFunction; stripeSubscriptionId: string }) {
  ...
  if (!tenantSubscription) {
    ...
    return Response.json("Account subscription not found", { status: 404 });
  }
  // eslint-disable-next-line no-console
  ...
  // notify if has ended
  if (data.currentPeriodEnd && data.currentPeriodEnd <= today && !tenantSubscription.endsAt) {
