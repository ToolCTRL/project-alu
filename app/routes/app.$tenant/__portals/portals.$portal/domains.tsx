import { ActionFunctionArgs, Form, Link, LoaderFunctionArgs, redirect, useActionData, useLoaderData, useNavigation, useParams, useSubmit } from "react-router";
...
export const action = async ({ request, params }: ActionFunctionArgs) => {
  ...
  if (action === "edit") {
    ...
    if (domain) {
      ...
      if (existingDomain) {
        return Response.json({ error: "Domain taken" }, { status: 400 });
      }
    ...
    if (certificate?.configured) {
      return Response.json({ success: "Domain verified" });
    } else {
      return Response.json({ error: "Domain not verified" }, { status: 400 });
    }
