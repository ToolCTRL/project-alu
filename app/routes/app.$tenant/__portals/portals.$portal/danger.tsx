...
export const action = async ({ request, params }: ActionFunctionArgs) => {
  ...
  if (action === "delete") {
    ...
    if (users.length > 0) {
      return Response.json({ error: "Cannot delete portal with users." }, { status: 400 });
    }
    if (products.length > 0) {
      return Response.json({ error: "Cannot delete portal with products." }, { status: 400 });
    }
...
export default function DangerRoute() {
