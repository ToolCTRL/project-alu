...
export const action: ActionFunction = async ({ request }) => {
  ...
  if (action === "delete") {
    ...
    try {
      ...
      return Response.json({ success: `Preview sent to channel: ` + channel });
    } catch (e: any) {
      return Response.json({ error: e.message }, { status: 500 });
    }
  }
  ...
  return (
    <InfoBanner title="Default In-app Notifications" text="">
      <div className="space-y-2">
        <div>
          Go to each notification template, and add a <i className="border-border border-b">Step</i> in the Workflow Editor of type{" "}
          <i className="border-border border-b">in-app</i> with the following fields:
          <ul>
            <li>
