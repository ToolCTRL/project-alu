...
export const action = async ({ request, params }: ActionFunctionArgs) => {
  ...
  if (action === "set-user-roles") {
    ...
    if (!isAdmin && countAdmins === 1) {
      return Response.json({ error: "You cannot remove admin access from the last admin user" }, { status: 400 });
    }
    const hasSuperAdmin = setRoles.some((r) => r.role.name === DefaultAdminRoles.SuperAdmin);
    if (user.id === userInfo.userId && !hasSuperAdmin) {
      return Response.json({ error: "You cannot remove super admin access from yourself" }, { status: 400 });
    }
