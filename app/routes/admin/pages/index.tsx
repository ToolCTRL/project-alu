...
export const loader = async ({ request, params }: LoaderFunctionArgs) => {
  ...
  await verifyUserHasPermission(request, "admin.pages.view", tenantId);
...
export const action: ActionFunction = async ({ request, params }) => {
  const { t } = await getTranslations(request);
  const tenantId = await getTenantIdOrNull({ request, params });
  await verifyUserHasPermission(request, "admin.pages.create", tenantId);
  if (tenantId) {
    return Response.json({ error: "Tenant-scoped admins cannot create or modify global pages" }, { status: 403 });
  }
  ...
  if (action === "create") {
    ...
    const existing = await getPageBySlug(finalSlug);
    if (existing) {
    ...
    if (created.length > 0) {
      return Response.json({ success: "Created default pages: " + created.map((f) => f.slug).join(", ") });
    } else {
      return Response.json({ error: "All default pages already exist: " + defaultPages.join(", ") }, { status: 400 });
    }
  } else {
      ...
      <div className="mx-auto mb-12 max-w-5xl space-y-5 px-4 py-4 sm:px-6 lg:px-8 xl:max-w-7xl">
        ...
        {pendingDefaultPages().length > 0 && data.items.length === 0 ? (
          <button
            ...
            className="border-border hover:border-border relative block w-full rounded-lg border-2 border-dashed p-12 text-center focus:outline-hidden focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
          >
