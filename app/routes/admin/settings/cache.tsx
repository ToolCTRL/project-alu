...
export const action = async ({ request, params }: ActionFunctionArgs) => {
  ...
  if (action === "delete-key") {
    ...
    cache.delete(key);
    return Response.json({ success: "Key deleted" });
  } else if (action === "delete-all") {
    let keyCount = cache.size;
    cache.clear();
    return Response.json({ success: "Cache cleared: " + keyCount + " keys deleted" });
    ...
    return Response.json({ success: "Keys deleted: " + keyCount });
  }
    ...
    <EditPageLayout
      title={
        <BackButtonWithTitle href="/admin/settings">
          {`Cache: ${NumberUtils.intFormat(data.cachedValues.length)} keys, ${NumberUtils.decimalFormat(
    ...
    >
      <div className="space-y-3">
        <div className="flex items-start space-x-4">
          <div className="w-52 shrink-0 space-y-2 p-1">
            <InputCombobox
              title="Group"
              name="keyGroupBy"
              // prefix="Key group"
