...
export const action = async ({ request, params }: ActionFunctionArgs) => {
  ...
  if (action === "bucket-create") {
    ...
    return Response.json({ success: "Bucket created" });
    ...
    return Response.json({ success: "Bucket updated" });
    ...
    if (bucketFiles.data?.length) {
      return Response.json({ error: "Bucket is not empty" }, { status: 400 });
    }
    const deleted = await deleteSupabaseBucket(id);
    if (deleted.error) {
      return Response.json({ error: deleted.error.name }, { status: 400 });
    }
    return Response.json({ success: "Bucket deleted" });
  } else {
      ...
      <div>
        {item ? (
          <>
            ...
            <input type="hidden" name="action" value="bucket-update" hidden readOnly />
          ...
          <>
            ...
            <input type="hidden" name="action" value="bucket-create" hidden readOnly />
          </>
        )}
      </div>
