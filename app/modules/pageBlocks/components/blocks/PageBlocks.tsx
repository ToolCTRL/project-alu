...
export default function PageBlocks({
  ...
  function onUpdateLayout(layout?: PageBlockDto["layout"]) {
    if (onChange) {
      onChange(
        items.map((b, i) => {
          if (i === editingBlockIndex) {
                ...
                {editor && (
                  ...
                  >
                    <div className="mx-auto flex w-full max-w-md items-center justify-between">
                      <div className="text-foreground text-lg font-extrabold capitalize">{getType(item)}</div>
    ...
    <div className={className}>
      <div className="relative">
        <div className="absolute inset-0 flex items-center" aria-hidden="true">
          <div className="border-theme-300 dark:border-border w-full border-t" />
        </div>
        <div className="relative flex justify-center">
          <button
            ...
            type="button"
            className={clsx(
              "border-theme-300 bg-theme-50 hover:bg-theme-100 focus:ring-ring dark:border-border dark:bg-theme-900 text-foreground/80 inline-flex items-center rounded-full border px-4 py-1.5 text-sm font-medium leading-5 shadow-2xs focus:outline-hidden focus:ring-2 focus:ring-offset-2",
              ...
              {Object.keys(PageBlockUtils.defaultBlocks).map((type) => {
                return (
                  <button
                    ...
                    className="border-border bg-secondary hover:bg-secondary/90 focus:bg-secondary/90 group flex w-full flex-col items-center space-y-2 rounded-md border-2 border-dashed p-2 hover:border-gray-800 focus:border-transparent focus:bg-gray-200 focus:outline-hidden focus:ring-2 focus:ring-gray-800"
                  >
