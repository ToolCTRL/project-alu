      ...
      <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8 lg:py-24">
        <div className="space-y-8 text-center sm:space-y-12">
          <div className="space-y-5 sm:mx-auto sm:max-w-xl sm:space-y-4 lg:max-w-5xl">
            ...
            <p className="text-muted-foreground text-xl">{t(item.subheadline)}</p>
            {item.cta && (
                    ...
                    <div key={item.href} className="rounded-md ">
                      {item.href.startsWith("http") ? (
                        <a
                          ...
                          rel="noreferrer"
                          className={clsx(
                            "hover:bg-secondary/90 group flex w-full items-center justify-center space-x-2 truncate rounded-md border px-8 py-3 text-base font-medium md:px-4 md:py-2 dark:hover:bg-gray-900"
                          )}
                        >
                          {t(item.text)}
                        </a>
            ...
            <div className="mx-auto lg:max-w-5xl">
              <ul className={item.grid ? GridBlockUtils.getClasses(item.grid) : "flex flex-wrap justify-center"}>
                {item.data.members.map((member) => {
                  const card = (
                    <li key={member.user}>
                      <div className="space-y-4">
                        <img
                          className="mx-auto h-10 w-10 shrink-0 rounded-full sm:h-12 sm:w-12 lg:h-14 lg:w-14"
                        ...
                        {item.withName && (
                          <div>
                            <div className="mt-2 text-xs font-medium lg:text-sm">
                              <h4 className="text-muted-foreground truncate">{member.user}</h4>
                            </div>
                          </div>
                        )}
                      </div>
                  ...
                  if (member.url) {
                    return (
                      <Link key={member.user} to={member.url} className="hover:underline" target="_blank">
                        {card}
                      </Link>
                    );
                  }
