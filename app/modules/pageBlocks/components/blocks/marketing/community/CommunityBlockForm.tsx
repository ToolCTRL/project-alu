              ...
              {state.data?.members?.map((member, index) => (
                <div key={member.user || index} className="group relative grid grid-cols-2 gap-2">
                  <button
                    ...
                    type="button"
                    className="text-muted-foreground bg-background absolute top-0 right-0 hidden origin-top-right justify-center rounded-full group-hover:flex hover:text-red-500"
                  >
                ...
                onClick={() =>
                  setState({
                    ...
                    data: {
                      ...
                      members: [
                        ...(state.data?.members ?? []),
                        { avatar_url: "https://placehold.co/100x100?text=Avatar", user: "User " + (state.data?.members?.length ?? 0 + 1) },
                      ],
                    },
                  })
                }
          ...
          {state.cta.map((cta, index) => (
            <div key={`cta-${index}-${cta.href}`} className="group relative grid grid-cols-2 gap-2">
              <button
                ...
                type="button"
                className="text-muted-foreground bg-background absolute top-0 right-0 -mt-3 hidden origin-top-right justify-center rounded-full group-hover:flex hover:text-red-500"
              >
