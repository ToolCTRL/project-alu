    ...
    <div>
      <div className="pb-6">
        <div className="relative pt-6 ">
          <div className={clsx("mx-auto px-4 sm:px-6", width === "screen-2xl" ? "max-w-(--breakpoint-2xl)" : "max-w-7xl")}>
                  ...
                  <div className="-mr-1 flex items-center space-x-2 md:hidden">
                    <div className="flex">
                      {item.withSignInAndSignUp && (
                        <div className="inline-flex space-x-2 rounded-md">
                          {/* "Kostenlos testen" Button entfernt - nur "Enter" Button */}
                    ...
                    {item.links.length > 0 && (
                      <button
                        ...
                        type="button"
                        className=" text-muted-foreground hover:bg-secondary hover:text-secondary-foreground inline-flex items-center justify-center rounded-md p-2 focus:outline-hidden"
                        id="main-menu"
                        aria-haspopup="true"
                      >
                    ...
                    <Fragment key={`nav-link-${link.title}-${idx}`}>
                      {!link.items || link.items.length === 0 ? (
                        <ButtonEvent
                          ...
                          className={clsx(
                            link.className,
                            "truncate rounded-sm px-3 py-1 text-base font-medium leading-6 transition duration-150 ease-in-out focus:outline-hidden",
                        ...
                        <HeaderFlyoutItem
                          className="rounded-sm px-3 py-1 text-base font-medium leading-6 transition duration-150 ease-in-out focus:outline-hidden"
                          title={t(link.title)}
                          items={link.items}
                        />
                      )}
              ...
              <div className="bg-background border-border overflow-visible rounded-lg border shadow-xl">
                <div className="flex items-center justify-between px-5 pt-4">
                  ...
                  <div className="-mr-2">
                    <button
                      onClick={() => setOpen(!open)}
                      type="button"
                      className="hover:bg-secondary hover:text-secondary-foreground inline-flex items-center justify-center rounded-md p-2 focus:outline-hidden"
                    >
                      <span className="sr-only">{t("shared.close")}</span>
                      {/* Heroicon name: x */}
                      <svg className="h-6 w-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
                ...
                <div role="menu" aria-orientation="vertical" aria-labelledby="main-menu">
                  ...
                  <div role="none" className="flex items-center px-2 pb-2">
                    {/* "Kostenlos testen" Button entfernt - nur "Enter/Sign In" Button */}
