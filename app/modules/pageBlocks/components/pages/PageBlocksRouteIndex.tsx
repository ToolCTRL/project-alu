...
export default function PageBlocksRouteIndex() {
  ...
  return (
    <Form method="post" className="relative h-[calc(100vh-100px)] space-y-3">
      ...
      <div className="border-border sticky top-0 w-full border-b py-2">
        <div className="flex items-center justify-center space-x-2">
          <ChatGptSetParametersButton
            page={data.page}
            className="focus:border-accent-300 border-border bg-background inline-flex items-center space-x-2 rounded-md border px-3 py-2 text-sm font-medium shadow-2xs focus:outline-hidden focus:ring-2"
            onGenerate={(info) => {
      ...
      <div className="mt-2">
        <div className="bg-background h-[calc(100vh-200px)] overflow-auto rounded-md shadow-lg dark:bg-gray-900">
          <PageBlocks
            page={data.page}
            items={blocks}
            editor={{
