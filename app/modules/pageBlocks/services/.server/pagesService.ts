...
export async function getPageConfiguration({
  ...
  if (!page) {
    page = await cachified({
      ...
      ttl: 1000 * 60 * 60 * 24,
      disabled: true,
      getFreshValue: () => getPageBySlug(slug),
    });
  }
...
export function parsePageBlocks({ t, slug, blocks }: { t: TFunction; slug: string; blocks: PageBlock[] }): PageBlockDto[] {
  ...
  if (parsedBlocks.length === 0) {
    if (slug === "/") {
      parsedBlocks = defaultLandingPage({ t });
    } else if (slug === "/pricing") {
      parsedBlocks = defaultPricingPage({ t });
    } else if (slug === "/blog") {
      parsedBlocks = defaultBlogPage({ t });
    } else if (slug === "/blog/:id1") {
      parsedBlocks = defaultBlogPostPage({ t });
    }
  }
  return parsedBlocks;
}
