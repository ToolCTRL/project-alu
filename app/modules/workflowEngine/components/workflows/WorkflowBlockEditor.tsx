...
export default function WorkflowBlockEditor({
  ...
  return (
    <div>
      <div className="">
        <div className="border-border flex h-12 justify-between border-b px-4 py-3 pr-6">
          <button
            type="button"
        ...
        <div className="space-y-4 px-4 py-4">
          <div className="space-y-2">
            <div className="flex items-center space-x-2">
              ...
              <div className="text-foreground/80 font-medium">{workflowBlock.name}</div>
            </div>
            <div>
              <input
                className=" text-muted-foreground hover:bg-secondary/90 w-full rounded-lg px-1 py-1 text-sm focus:outline-hidden"
                ...
                onBlur={(e) => {
                  if (e.target.value !== block.description) {
          ...
          {errors.length > 0 && (
            <Fragment>
              ...
              <div className="space-y-3">
                <div className="text-foreground/80 text-sm font-medium">Errors</div>
          ...
          {workflowBlock.value === "if" && (
            <Fragment>
              <div className="border-border border-t"></div>
              <div className="space-y-3">
                <div className="text-foreground/80 text-sm font-medium">Conditions</div>
          ...
          {workflowBlock.value === "switch" && (
            <Fragment>
              <div className="border-border border-t"></div>
              <div className="space-y-3">
                <div className="text-foreground/80 text-sm font-medium">Conditions</div>
                <div className="space-y-1">
                  <ConditionsGroupsInfo
                    workflow={workflow}
                    block={block}
                    type="switch"
                    onUpdateConditionsGroups={(i) => onUpdateConditionsGroups(block.id, i)}
                  />
                </div>
              </div>
            </Fragment>
          ...
          {workflowBlock.inputs.length > 0 && (
            <Fragment>
              <div className="border-border border-t"></div>
              <div className="space-y-3">
                <div className="text-foreground/80 text-sm font-medium">Inputs</div>
                <div className="space-y-1">
          ...
          {workflowBlock.outputs.length > 0 && (
            <Fragment>
              <div className="border-border border-t"></div>
              <div className="space-y-3">
                <div className="text-foreground/80 text-sm font-medium">Outputs</div>
                <div className="space-y-1">
...
function BlockInput({
  ...
  }, [debouncedInputValue, onChange, block.input, input.name]);
  ...
  return (
    <div key={input.name} className=" space-y-1">
      <div className="flex items-center justify-between space-x-2">
        <label htmlFor={"input-" + input.name} className="text-muted-foreground flex justify-between space-x-2 truncate text-xs font-medium">
          {input.label}{" "}
          {input.href && (
      ...
      {input.type === "select" && (
        <select
          name={input.name}
          id={input.name}
          className="border-border bg-secondary hover:bg-secondary/90 w-full rounded-lg border px-2 py-2 text-sm focus:outline-hidden"
          value={inputValue as string}
        ...
        >
          ...
          {input.options
            ?.filter((f) => !f.adminOnly || !params.tenant)
            .map((option) => (
...
function BlockOutput({ block, output }: { block: WorkflowBlockDto; output: WorkflowBlockOutput }) {
  return (
    <div key={output.name}>
      <div className="flex items-center justify-between space-x-2">
        <label htmlFor={"output-" + output.name} className="mb-1 flex justify-between space-x-2 truncate text-xs font-medium">
          {output.label}
        </label>
      </div>
      <input
  ...
  if (!nextBlock) {
    return (
      <div className="space-y-0.5">
        <label className="text-muted-foreground flex justify-between space-x-2 truncate text-xs font-medium uppercase">{title}</label>
        <button
          type="button"
    ...
    <div className="space-y-0.5">
      ...
      >
        <div className="flex justify-between space-x-2 text-sm font-medium">
          <div>
            {workflowBlock.name}{" "}
            {nextWorkflowBlock.description && <span className="text-muted-foreground text-xs font-normal">({nextWorkflowBlock.description || ""})</span>}
          </div>
          <button
            type="button"
            onClick={(e) => {
