    ...
    <div>
      <div className="space-y-3">
        <div className="space-y-0.5 text-sm">
          <div className="text-foreground/80 font-medium">Input examples</div>
        ...
        <div className="space-y-2">
          {workflow.inputExamples.map((inputExample) => {
            return (
              <button
                type="button"
                key={inputExample.id}
                className="border-border bg-secondary hover:bg-secondary/90 w-full rounded-md border border-dotted p-2 hover:border-dashed"
                onClick={() => {
                  setAddingInputExample(false);
              ...
              >
                <div className="flex justify-between space-x-2">
                  <div className="font-sm text-foreground/80 shrink-0 text-xs">{inputExample.title}</div>
                  <div className="text-muted-foreground truncate text-xs">{JSON.stringify(inputExample.input, null, 2)}</div>
                </div>
              </button>
            );
          })}
          <button
            type="button"
            className="hover:bg-secondary border-border bg-background w-full rounded-md border border-dotted p-2 hover:border-dashed"
            onClick={() => {
              setAddingInputExample(true);
              setSelectedInputExample(null);
            }}
          >
            <div className="font-sm text-foreground/80 text-xs">- Add input example -</div>
          </button>
        </div>
      </div>
