  ...
  return (
    <Fragment>
      <button
        ...
        className="border-border text-muted-foreground hover:text-foreground hover:bg-background bg-secondary rounded-md border p-1 text-xs font-medium focus:outline-hidden focus:ring-2 focus:ring-gray-500 focus:ring-offset-2"
      >
...
function VariableList({ workflow, block, onSelected }: { readonly workflow: WorkflowDto; readonly block: WorkflowBlockDto; readonly onSelected: (variable: { name: string }) => void }) {
  ...
  useEffect(() => {
    ...
    if (workflow.$credentials && workflow.$credentials.length > 0) {
      groups.unshift({
        name: "Credentials",
        variables: workflow.$credentials.map((f) => ({
          name: `{{$credentials.${f}}}`,
          label: f,
        })),
      });
    ...
    const variableBlocks = workflow.blocks.filter((x) => x.type === "variable");
    variableBlocks.forEach((variableBlock) => {
      globalVariables.push(variableBlock.input.variableName);
    });
            ...
            <div className="space-y-1">
              {group.variables.map((variable) => (
                <button
                  type="button"
                  key={variable.name}
                  className="hover:bg-secondary border-border bg-background flex w-full items-center space-x-2 truncate rounded-md border px-2 py-2 text-sm"
