  ...
  return (
    <button
      ...
      className={clsx(
        ...
        !workflowBlockExecution && "bg-background border-border focus:ring-blue-600",
      ...
      <div className="w-full space-y-2">
        <div className="flex justify-between space-x-2">
          <div className="flex items-center space-x-2 truncate">
            ...
            <div className="text-foreground/80 truncate text-sm font-medium">{workflowBlock.name}</div>
          </div>
          <div>
            <div
              className={clsx(
                "rounded-md border px-2 py-1 text-xs",
                workflowBlock.type === "trigger" ? "border-yellow-200 bg-yellow-50 text-yellow-800" : "border-border text-muted-foreground bg-secondary"
              )}
            >
        ...
        {errors.length > 0 ? (
          ...
          <div className=" text-muted-foreground line-clamp-1 text-xs">{block.description || "No description"}</div>
        )}
