...
async function validate(tenantId: string | null = null) {
  const crmEntities = ["company", "opportunity", "contact", "ticket"];
  ...
  await Promise.all(
    crmEntities.map(async (crmEntity) => {
      ...
      if (tenantId === null && ![DefaultEntityTypes.All, DefaultEntityTypes.AdminOnly].includes(entity.type as DefaultEntityTypes)) {
        throw new Error(`CRM is not configured: Entity "${crmEntity}" must be "All" or "Admin"`);
      } else if (tenantId !== null && ![DefaultEntityTypes.All, DefaultEntityTypes.AppOnly].includes(entity.type as DefaultEntityTypes)) {
