...
import { ApiSpecsDto } from "../services/ApiSpecsService";
  ...
  return (
    <div className={clsx("grid gap-4", item.postmanCollection ? "grid-cols-2" : "grid-cols-1")}>
      <div className="space-y-2">
        {withLinkToDocs && (
          <Link to="/api/docs" target="_blank">
            <div className="hover:border-border hover:bg-background rounded-md border border-transparent px-2 py-1.5">
              <div className="flex items-center justify-between space-x-2">
                <div className="flex items-center space-x-2">
                  <svg className="h-5 w-5" xmlns="http://www.w3.org/2000/svg" width="256" height="256" preserveAspectRatio="xMidYMid" viewBox="0 0 256 256">
                    <path
                      fill="#fff"
                    ...
                    <path
                      fill="#fff"
                      d="M169.327 127.956c-.284 5.29-4.906 9.683-9.46 8.917h-.069a9.133 9.133 0 0 1-9.145-9.124 9.276 9.276 0 0 1 9.427-8.96c5.045.025 9.52 4.288 9.247 9.167zM88.201 179.22c1.902.056 3.806.015 6 .015v13.786c-13.635 2.305-24.866-1.566-27.622-13.091a76.501 76.501 0 0 1-1.736-12.886c-.293-4.592.213-9.235-.137-13.819-.97-12.612-2.603-16.918-14.706-17.514v-15.696a22.665 22.665 0 0 1 2.632-.452c6.636-.327 9.433-2.362 10.917-8.898a74.57 74.57 0 0 0 1.194-11.122c.526-7.217.34-14.551 1.542-21.651 1.737-10.267 8.111-15.255 18.64-15.814 2.996-.16 6.001-.024 9.396-.024v14.09c-1.397.1-2.677.303-3.95.265-8.58-.263-9.023 2.66-9.65 9.762-.39 4.454.149 8.984-.155 13.454a107.113 107.113 0 0 1-1.781 13.245c-1.238 6.339-5.135 11.053-10.535 15.053 10.483 6.823 11.676 17.423 12.357 28.188.366 5.784.199 11.61.786 17.366.457 4.467 2.195 5.607 6.808 5.743zm8.836-60.43h.162a9.078 9.078 0 0 1 8.928 9.226 8.884 8.884 0 0 1-9.396 8.851 9.046 9.046 0 1 1 .306-18.078zm31.236 0c5.483-.043 9.124 3.51 9.152 8.93.03 5.565-3.422 9.125-8.868 9.149-5.54.024-9.186-3.48-9.216-8.867a8.671 8.671 0 0 1 8.933-9.213zm65.4-6.867c1.459 5.447 4.298 7.362 10.032 7.623.94.044 1.875.203 3.163.346v15.692c-.697.23-1.412.4-2.138.512-7.684.478-11.186 3.63-11.962 11.335-.496 4.919-.455 9.892-.796 14.828a125.806 125.806 0 0 1-1.476 16.18c-1.96 9.703-8.019 14.544-18.029 15.135-3.221.19-6.466.03-9.94.03v-14.026c1.87-.115 3.52-.275 5.174-.314 5.981-.143 8.096-2.071 8.389-8.011.324-6.525.465-13.059.757-19.586.423-9.433 3.006-17.861 11.795-23.745-5.03-3.585-9.067-7.928-10.112-13.783-1.265-7.097-1.674-14.351-2.355-21.548-.336-3.597-.32-7.226-.671-10.821-.379-3.88-3.045-5.224-6.577-5.31-2.024-.05-4.055-.01-6.642-.01V62.754c16.509-2.741 27.913 2.752 28.973 18.548.443 6.632.378 13.297.803 19.93.186 3.608.726 7.19 1.612 10.691z"
                    />
                  </svg>
                  <div className="text-foreground truncate text-sm font-medium">View OpenAPI 3.0 Docs</div>
                </div>
                <ExternalLinkEmptyIcon className="ml-2 h-4 w-4" />
              </div>
            </div>
          </Link>
        )}
        <div className={clsx(className)}>
          <MonacoEditor value={JSON.stringify(item.openApi, null, 2)} theme="vs-dark" language="json" />
        </div>
      </div>
      ...
      {item.postmanCollection && (
        <div className="space-y-2">
          {withLinkToDocs && (
            <Link to="/postman_collection.json" target="_blank">
              <div className="hover:border-border hover:bg-background rounded-md border border-transparent px-2 py-1.5">
                <div className="flex items-center justify-between space-x-2">
                  <div className="flex items-center space-x-2">
                    <svg className="h-4 w-4" xmlns="http://www.w3.org/2000/svg" width="800" height="800" preserveAspectRatio="xMidYMid" viewBox="0 0 256 256">
                      <path
                        fill="#FF6C37"
                    ...
                    <div className="text-foreground truncate text-sm font-medium">Download Postman Collection</div>
                  </div>
                  <ExternalLinkEmptyIcon className="ml-2 h-4 w-4" />
                </div>
              </div>
          ...
          <div className={clsx(className)}>
            <MonacoEditor value={JSON.stringify(item.postmanCollection, null, 2)} theme="vs-dark" language="json" />
          </div>
        </div>
      )}
    </div>
