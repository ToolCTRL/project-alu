...
export default function FormulaComponentBadge({ item }: FormulaComponentBadgeProps) {
  return (
    <div>
      ...
      {item.type === "operator" && <span className="text-foreground text-sm font-bold">{FormulaHelpers.getOperatorSymbol(item.value)}</span>}
      {item.type === "parenthesis" && <span className="text-muted-foreground">{item.value === "OPEN" ? "(" : ")"}</span>}
