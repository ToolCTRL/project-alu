...
export default function KbDocsLayout({
  ...
  return (
    <div>
      <KbDocsHeader kb={kb} />
      <div className="mx-auto max-w-(--breakpoint-2xl) flex-1 items-start px-4 pt-3 sm:px-6 md:grid md:grid-cols-[220px_minmax(0,1fr)] md:gap-6 lg:grid-cols-[240px_minmax(0,1fr)] lg:gap-10">
        <nav className={clsx("not-prose border-border mb-2 flex items-center truncate border-b pb-2 md:hidden")}>
          <ol className="flex items-center space-x-2">
            <MobileNav items={items} />
            {currentItem() && (
              <Fragment>
                <RightIcon className="text-muted-foreground h-4 w-4 shrink-0" />
                <div>
