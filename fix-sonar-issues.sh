...
find app/routes -name "*.tsx" -type f | while read file; do
  if grep -q "import { serverTimingHeaders } from \"~/modules/metrics/utils/defaultHeaders.server\";" "$file" 2>/dev/null; then
    if grep -q "^export { serverTimingHeaders as headers };$" "$file" 2>/dev/null; then
...
echo "Fixing S7723: new Error() constructor..."
find app/routes -name "*.tsx" -type f | while read file; do
...
echo "  Total S7723 fixes: $FIXED_S7723"
echo ""
...
echo "=== Summary ==="
echo "S7763 (export...from): $FIXED_S7763 files"
echo "S7723 (new Error): $FIXED_S7723 files"
echo "S7726 (named functions): Partially completed"
echo ""
echo "=== Done ==="
